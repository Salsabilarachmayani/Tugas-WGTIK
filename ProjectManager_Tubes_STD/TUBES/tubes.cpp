...
void showProject(listTask LT, string status, adrTask T) {
    T = searchTask(LT, status);
    if (child(T) == nil) {
        cout << "Task Tidak Ditemukan!" << endl;
    }else{
        ...
        while (P != nil) {
            cout << infoP(P).namaProject << " " << infoP(P).tanggal.day << "/" << infoP(P).tanggal.month << "/" << infoP(P).tanggal.year << " " << infoP(P).status << endl;
            P = nextP(P);
        }
    }
    cout << endl;
